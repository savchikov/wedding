// ============================
// Редирект в браузер из Telegram
// ============================
(function(){
  if (!window.TelegramWebview && !/Telegram|TelegramBot/i.test(navigator.userAgent) && !window.location.href.includes('tgWebAppData')) return;
  var u = location.href, ios = /iPad|iPhone|iPod/.test(navigator.userAgent), and = /Android/.test(navigator.userAgent);
  try {
    if (and) { location.href = 'intent://' + u.replace(/^https?:\/\//, '') + '#Intent;scheme=https;package=com.android.chrome;end'; }
    else if (ios) { var w = window.open(u, '_blank'); if (!w) location.href = u; }
